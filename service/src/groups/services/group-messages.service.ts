...
export class GroupMessageService implements IGroupMessageService {
    ...
    async createGroupMessage({ groupId: id, ...params }: CreateGroupMessageParams) {
        ...
        const updateGroup = await this.groupService.saveGroup(group);
        return { message: saveMessage, group: updateGroup };
    }
    ...
    async editGroupMessage(params: EditGroupMessageParams) {
        ...
        if (!messageDB) {
            throw new HttpException('Connot edit Message', HttpStatus.BAD_REQUEST);
        }
